resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              = "web-${var.environment}.${var.zone_name}" #web-dev.puneeth.cloud
  ...
  enabled             = true
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]  #web-cdn.puneeth.cloud
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.puneeth.cloud
      type    = "A"
